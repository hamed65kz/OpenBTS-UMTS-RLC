...
void URlc::macPushUpRXDCCH(char *pdu, int pdu_len, RbId rbid, UeIdType id_type,
                           int UEid, int nodeBIndex = 0) {
  // URNTI = 0;
  // CRNTI = 1;
...
void URlc::rrcSendRRCConnectionSetup(uint32_t urnti, uint16_t crnti, char *sdu,
                                     int sdu_len, int nodeBIndex = 0) {
  if (nodeBIndex >= mNodeBCount) {
    LOG(WARNING) << "NodeB " << nodeBIndex << " doesn exist";
    return;
  }
  LOG(INFO) << "rrcSendRRCConnectionSetup urnti = " << urnti
            << " crnti = " << crnti << " nodeB = " << nodeBIndex;
...
void URlc::rrcSendDCCH(char *sdu, int sdu_len, UeIdType id_type, int UEid,
                       RbId rbid, std::string desc, int nodeBIndex = 0) {
  if (nodeBIndex >= mNodeBCount) {
    LOG(WARNING) << "NodeB " << nodeBIndex << " doesn exist";
    return;
  }
  ...
  UEInfo *uep = gRrc.findUe(ueidtype, UEid, nodeBIndex);
  if (uep == NULL) {
    LOG(INFO) << "Could not find UE with id " << UEid << " in nodeb "
              << nodeBIndex;
    return;
  }

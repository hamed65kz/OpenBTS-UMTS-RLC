...
#ifdef _WIN32
#define HAVE_STRUCT_TIMESPEC // for resolve timespec redefinition,, msvc version
                             // // should be higher than vs2013
  ...
  while (true) {
    ...
    for (int i = 0; i < pdus.size(); i++) {
      ...
      cout << "<Tx Pdu Received on MAC"
           << " size : " << pdus[i]->payload_length
           << " paylod : " << pdus[i]->payload_string << "\n";
    }
...
void *rrcRxCCCH(void *) {
  while (true) {
    ...
    if (sdu->payload_length > 0) {
      ...
      URlc::rrcSendRRCConnectionSetup(urnti, crnti, sdu->payload,
                                      sdu->payload_length, rand() % 3);
      delete[] sdu->payload;
    }
    delete sdu;
    SLEEP(500);
